...
const Fertilizers = () => {
  ...
  const handlePageChange = async (newPage) => {
    ...
    const response = await searchFertilizerAidInfo(formData, newPage, 10)
  ...
  const handleDelete = async (aid_id) => {
    ...
    //     }
    //   })
    const response = await deleteFertilizerAid(aid_id)
  ...
  const handleSubmit = async () => {
    ...
    const response = await searchFertilizerAidInfo(formData, currentPage, 10)
    setAids(response.data.data)
    console.log(response.data.data)
    setTotalPages(response.data.total_pages)
    console.log(response.data.total_pages, 'totalPages')
  ...
  const handleNewItemAddButtonSubmit = (event) => {
    ...
    axios
      .post(`${API_BASE_URL}/aid/fertilizer`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then(function (response) {
        ...
        alert('New Fertilizer was added Successfully!')
      })
      .catch(function (error) {
        console.error(error)
  ...
  const handleUpdate = async () => {
    const response = await updateFertilizerAid(currentRecord)
    console.log(response)
    if (response.status === 200) {
      ...
      {showForm ? (
        <CContainer>
          <CRow className="justify-content-end">
            <CCol xs="auto">
              <CButton color="danger" onClick={handleAddFertilizer}>
                Close
              </CButton>
            </CCol>
          </CRow>
        ...
        <CContainer>
          <CRow className="justify-content-center">
            <CCol xs="auto" className={'mb-3'}>
              <CButton color="success" onClick={handleAddFertilizer}>
                + Add New Fertilizer
              </CButton>
            </CCol>
          </CRow>
                    ...
                    <CTable>
                      <CTableHead>
                        <CTableRow>
                          ...
                          <CTableHeaderCell>Fertilizer ID</CTableHeaderCell>
                      ...
                      <CTableBody>
                        {aids.map((aid) => (
                          <CTableRow key={aid.fertilizer_id}>
                            <CTableDataCell>{aid.aid_id}</CTableDataCell>
                            <CTableDataCell>{aid.fertilizer_id}</CTableDataCell>
                        ...
                        <CForm>
                          ...
                          >
                            ...
                            <CFormInput
                              ...
                              autoComplete="Fertilizer ID"
                              onChange={handleUpdateInputChange}
                              name="fertilizer_id"
                              value={currentRecord.fertilizer_id}
                            />
                          </CInputGroup>
                          <CInputGroup
                            className={`mb-3 ${isFormEmpty ? 'border border-danger' : ''}`}
